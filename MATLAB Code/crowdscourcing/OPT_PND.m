...
load("result/result-10/data.mat")
load("result/result-10/distance_all_attributes.mat")
load("result/result-10/overall_accuracy_final_ori.mat")
load("result/result-10/overall_accuracy_ori.mat")
load("result/result-10/quality.mat")
...
loss_OPT_PND=zeros(length(epsilon_allocation_upper),1);
for epsilon_id=1:length(epsilon_allocation_upper)
    epsilon_allocat=epsilon_allocation_upper(epsilon_id,:);
    % method_obfus=[1,1,1,3]; 
    threshold_adj=[1, 0.125, 0.15, 0.15];
    %loss_set1_OPT_PND=cal_loss_benders(epsilon_allocat(1),distance_all_attributes{1,1},cost_attribute{1,1},1);
    % 1
    ...
    %loss_OPT_PND(epsilon_id,1)=1.00859*loss_set1_OPT_PND+0.991475*loss_set2_OPT_PND+0.736224*loss_set3_OPT_PND+1.00261*loss_set4_OPT_PND;
    loss_OPT_PND(epsilon_id,1)=loss_set1_OPT_PND+loss_set2_OPT_PND+loss_set3_OPT_PND+loss_set4_OPT_PND;
end

save('result/result-10/loss_OPT_PND.mat','loss_OPT_PND');
loss_OPT_PND_lower=loss_lower;
save('result/result-10/loss_OPT_PND_lower.mat','loss_OPT_PND_lower');