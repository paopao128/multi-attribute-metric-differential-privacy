...
loss_LP=zeros(1,length(epsilon));
for epsilon_id=1:length(epsilon)
    %% Parameters 
    NR_RECORD = length(cost); 
    
    EPSILON = epsilon(epsilon_id); 
    DP_THRESHOLD = 2; 
    ...
    for k = 1:1:NR_RECORD
        for i = 1:1:NR_RECORD
            for j = (i+1):1:NR_RECORD
                    ...
                    a_value = exp(EPSILON * distance_matrix(i,j)/2);
