...
% Use X directly (already preprocessed)
all_attrs = 1:size(X, 2);
H_clean = calculate_entropy_metrics(X, all_attrs);
...
function H = calculate_shannon_entropy(x)
    ...
    % Works directly with continuous or discrete data
    ...
    % Remove zero probabilities (shouldn't happen, but for safety)
    probs = probs(probs > 0);
...
function H_joint = calculate_joint_entropy(X)
    ...
    % Treat each unique row as a joint outcome
    [~, ~, idx] = unique(X, 'rows');
    n_unique = max(idx);
    
    % Calculate probability for each unique combination
    probs = zeros(1, n_unique);
    for i = 1:n_unique
